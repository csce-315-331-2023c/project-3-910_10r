...
function App() {
  ...
  const [payPage, setPayPage] = useState<boolean>(storedPayPage ? JSON.parse(storedPayPage) : false);
  const [isManager, setIsManager] = useState<boolean>(storedIsManager ? JSON.parse(storedIsManager) : false);
  const [isCashier, setIsCashier] = useState<boolean>(storedIsCashier ? JSON.parse(storedIsCashier) : false);
  const [isLogin, setIsLogin] = useState<boolean>(storedIsLogin ? JSON.parse(storedIsLogin) : true);
  const [num, setNumber] = useState<number>(storedNum ? JSON.parse(storedNum) : 1);
  ...
  return (
      <Routes>
        ...
        <Route path='/manager' element={isManager ? <Manager setIsLogin={setIsLogin} setIsManager={setIsManager} setIsCashier={setIsCashier} /> : <Navigate to="/payment"/>} >
        </Route>
        <Route path='/payment' element={payPage ? <Payment setPayPage={setPayPage} num={num} setIsCashier={setIsCashier}/> : <Navigate to="/cashier" />}>
        </Route>
        <Route path='/cashier' element={isCashier ? <Cashier setPayPage={setPayPage} setIsLogin={setIsLogin} setNumber={setNumber} num={num} setIsManager={setIsManager} setIsCashier={setIsCashier}/> : payPage ? <Navigate to="/payment"/> : <Navigate to="/"/>}>
        </Route>
